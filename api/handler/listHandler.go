...
// UpdateList handles PUT list/update requests.
...
func (h *ListHandler) GetList(w http.ResponseWriter, r *http.Request) {
	...
	if username == "" || username == "undefined" {
		log.Printf("query parameter for username")
		errorutils.Write(w, "no query parameter for username", http.StatusBadRequest)
		return
	}
	...
	log.Println(string(finalResponse))
...
func (h *ListHandler) DeleteListItem(w http.ResponseWriter, r *http.Request) {
	...
	if username == "" || gameId == "" {
		log.Printf("query parameter for username or gameId is missing")
		errorutils.Write(w, "", http.StatusBadRequest)
		return
	}
	gameIdInt, err := strconv.Atoi(gameId)
	if err != nil {
